...
--
-- Hôte : 127.0.0.1
-- Généré le :  mar. 14 mai 2019 à 16:43
...
--
DELIMITER $$
CREATE TRIGGER `test_sexe` BEFORE INSERT ON `athlete` FOR EACH ROW BEGIN
DECLARE SEXE_incorect CONDITION FOR SQLSTATE '22018';
DECLARE EXIT HANDLER FOR SEXE_incorect
RESIGNAL set MESSAGE_TEXT ='ENTRER M ou F';
IF NEW.sexe != 'M' AND NEW.sexe != 'F'
THEN SIGNAL SEXE_incorect;
END IF;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `test_date` BEFORE INSERT ON `epreuve` FOR EACH ROW BEGIN
DECLARE date_epreuve_incorect CONDITION FOR SQLSTATE '22008';
DECLARE EXIT HANDLER FOR date_epreuve_incorect
RESIGNAL set MESSAGE_TEXT ='ENTRER une date de début plus petite que la date de fin';
IF NEW.DATE_DEBUT_EPREUVE > NEW.DATE_FIN_EPREUVE
THEN SIGNAL date_epreuve_incorect;
END IF;
END
$$
DELIMITER ;
...
--
-- Déclencheurs `session`
--
DELIMITER $$
CREATE TRIGGER `test_date_session` BEFORE INSERT ON `session` FOR EACH ROW BEGIN
    IF NEW.DATE_DEBUT_SESSION > NEW.DATE_FIN_SESSION
        THEN
        SET NEW.DATE_DEBUT_SESSION = NULL;
        SET NEW.DATE_FIN_SESSION = NULL;
    END IF;
END
$$
DELIMITER ;
